...
async def claim_parcel_api(request: Request, parcel_request: ClaimRequest = Depends()):
    ...
    overlapping_response_dto_list: list[OverlappingResponseDTO] = validate_claim_parcel(polygon, body.state_code)
...
# @dispatch(dispatch_event=None)
def claim_parcel_query_api(request: Request, claim_query_request: ClaimParcelQueryRequest = Depends()):
...
def validate_claim_parcel(polygon_wkt, state_code):
    # 1- validate if polygon is in a containing cms polygon TODO: what if there are overlaps with multiple cms
    polygon_wrapper_req_dto = PolygonWrapperCmsDTO(state_code=state_code, polygon_wkt=polygon_wkt)
